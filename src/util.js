...
export function createGroundPlaneXZ(width, height, widthSegments = 10, heightSegments = 10, gcolor = null) {
    ...
    // plane.matrix.multiply(mat4.makeTranslation(0.0, 0.0, 0.0)); // T1   
    // plane.matrix.multiply(mat4.makeRotationX(degreesToRadians(-90))); // R1  
 
    return plane;
 }

 export function translateMesh(obj, x = 0, y = 0, z = 0) {
    let mat4 = new THREE.Matrix4(); // Aux mat4 matrix   
    // obj.receiveShadow = true;
    // Rotate 90 in X and perform a small translation in Y
    obj.matrixAutoUpdate = false;
    obj.matrix.identity();    // resetting matrices
    // Will execute R1 and then T1
    obj.matrix.multiply(mat4.makeTranslation(x, y, z)); // T1   
    obj.matrix.multiply(mat4.makeRotationX(degreesToRadians(-90))); // R1   
 
 }

 export function rotateMesh(obj, angleDeg = 0) {
    let mat4 = new THREE.Matrix4(); // Aux mat4 matrix   
    // obj.receiveShadow = true;
    // Rotate 90 in X and perform a small translation in Y
    obj.matrixAutoUpdate = false;
    obj.matrix.identity();    // resetting matrices
    // Will execute R1 and then T1
    // obj.matrix.multiply(mat4.makeTranslation(0.0, 0.0, 0.0)); // T1   
    obj.matrix.multiply(mat4.makeRotationX(degreesToRadians(-90 + angleDeg))); // R1   
 
 }

 export function getTree(x, z) {
